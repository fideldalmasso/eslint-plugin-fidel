  ...
  create(context) {
    return {
      ...
      'CallExpression[callee.name=/(GraphOp|graphqlOperation)/]': async function (node) {
        ...
        const args = node.arguments[1] && node.arguments[1].properties?node.arguments[1].properties : null;
        if (!args || !args[0].key || !args[0].key.name) {
          if (DEBUG_MODE)
            console.log(
              `Object argument from query couldn't be analyzed. The obj it's probably defined somewhere else`
            );
          return;
        }
        let argNames = [];
